...
def add_shift():
    if request.method == 'POST':
        ...
        end_day = day if end_time > start_time else day + 1
...
def confirm_sequence():
    ...
    # Step 2: Confirm sequences and send notifications to all workers
    ...
    # Step 5: Send notification to the first worker in each sequence
    for wg in workgroups:
        for worker_type in ['senior', 'junior']:
            first_worker_sequence = WorkerSequence.query.filter_by(
                workgroup_id=wg.id,
                type=worker_type,
                status='confirmed',
                sequence=1
            ).first()
            if first_worker_sequence:
                Notification.send_notification(
                    recipient_id=first_worker_sequence.worker_id,
                    recipient_type='worker',
                    message='It is your turn to choose schedules.'
                )
    
    # Step 6: Send file for download
    return send_file(
        output,
        as_attachment=True,
...
def export_schedule_by_worker():
    ...
    for worker in workers:
        ...
        shifts = [
            f"{schedule.shift.type}: {(schedule.shift.day)} "
...
@main.route('/admin/export_schedule_in_spreadsheet', methods=['GET'])
@admin_required
def export_schedule_in_spreadsheet():
    # Get unique days from Shift model
    unique_days = sorted(set(shift.day for shift in Shift.query.all()))
    
    # Fetch all workers and their workgroups
    workers = Worker.query.all()
    worker_data = {}
    for worker in workers:
        sequence = WorkerSequence.query.filter_by(worker_id=worker.id).first()
        workgroup_name = sequence.workgroup.name if sequence and sequence.workgroup else 'None'
        worker_data[worker.id] = {
            'name': worker.name,
            'workgroup': workgroup_name,
            'schedules': {day: [] for day in unique_days}  # Initialize empty lists for each day
        }
    
    # Fetch schedules and map shifts to workers and days
    ...
    for schedule in schedules:
        ...
        if worker_id in worker_data and day in worker_data[worker_id]['schedules']:
            worker_data[worker_id]['schedules'][day].append(shift_type)
    
    # Prepare CSV data
    data = []
    for worker_id, info in worker_data.items():
        row = {
            'Worker Name': f"{info['name']} ({info['workgroup']})"
        }
        for day in unique_days:
            shifts_on_day = ', '.join(info['schedules'][day]) if info['schedules'][day] else 'None'
            row[f'Day {day}'] = shifts_on_day
        data.append(row)
    
    # Generate CSV in memory
    si = StringIO()
    fieldnames = ['Worker Name'] + [f'Day {day}' for day in unique_days]
    writer = csv.DictWriter(si, fieldnames=fieldnames)
    writer.writeheader()
    writer.writerows(data)
    ...
    return Response(
        generate(),
        mimetype='text/csv',
        headers={"Content-Disposition": "attachment; filename=schedule_spreadsheet.csv"}
    )
...
def reset_schedule():
    ...
    # Reset current worker counts in all intervals
    intervals = Interval.query.all()
    for interval in intervals:
        interval.current_senior_workers = 0
        interval.current_junior_workers = 0
    ...
    AdminOperationLog.log_operation(
        ...
        entity_type="system",
        entity_id=None,
        details="Deleted all previous WorkerSequence, Schedule, and Notification data, reset traded_points for all workers, and reset interval worker counts."
    )
...
# edit points (for exceptions, such as the change of schedule or not presenting)


@main.route('/admin/update_worker_points', methods=['GET', 'POST'])
@admin_required  # Restricts access to admins only
def update_worker_points():
    if request.method == 'POST':
        # Extract form data
        ...
        # Validate inputs
        if not worker_id or not points_str:
            flash('Please provide both worker ID and points.', 'error')
            return redirect(url_for('main.update_worker_points'))
        ...
        old_points = worker.points
        worker.points = new_points
        db.session.commit()

        # Log the modification in AdminOperationLog
        details = f"Updated points from {old_points} to {new_points}"
        AdminOperationLog.log_operation(
            admin_id=current_user.id,
        ...
        message = f"Your points have been updated from {old_points} to {new_points} by admin {current_user.name}."
        Notification.send_notification(
            recipient_id=worker.id,
            recipient_type='worker',
            message=message
        ...
        flash('Worker points updated successfully!', 'success')
        return redirect(url_for('main.update_worker_points'))

    # For GET requests, display the form with all workers
    workers = Worker.query.all()
    return render_template('admin/update_worker_points.html', workers=workers)
...
def schedule_system():
    ...
    if request.method == 'POST':
        ...
        for shift_id in selected_shift_ids:
            ...
            for existing_shift in selected_shifts:
                if (shift.day == existing_shift.day and
                    ...
                    return render_template('worker/schedule_system.html', shifts=shifts, worker_type=worker_type)
        ...
        if next_worker:
            ...
            Notification.send_notification(
                recipient_id=0000,  # Testing Admin ID
                recipient_type='admin',
    ...
    # Pass shifts to the template for GET request
    return render_template('worker/schedule_system.html', shifts=shifts, worker_type=worker_type)
