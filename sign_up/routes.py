...
def shift_interval_import():
    if request.method == 'POST':
        ...
        try:
            ...
            workgroup_ids = set(w.id for w in Workgroup.query.all())  # Correctly defines 'w'
            ...
            existing_pairs = set(db.session.query(
                ShiftInterval.shift_id, ShiftInterval.workgroup_id, ShiftInterval.interval_id
            ).all())
            ...
            for index, row in df.iterrows():
                ...
                if shift_id not in shift_ids:
                    errors.append(f"Invalid shift_id {shift_id} at row {index+1}")
                elif workgroup_id not in workgroup_ids:
                    errors.append(f"Invalid workgroup_id {workgroup_id} at row {index+1}")
                elif interval_id not in interval_ids:
                    errors.append(f"Invalid interval_id {interval_id} at row {index+1}")
                elif interval_workgroups.get(interval_id) != workgroup_id:
                    errors.append(f"Interval {interval_id} does not belong to workgroup {workgroup_id} at row {index+1}")
                elif (shift_id, workgroup_id, interval_id) in existing_pairs:
                    errors.append(f"Relationship ({shift_id}, {workgroup_id}, {interval_id}) already exists at row {index+1}")
                else:
                    new_relationships.append((shift_id, workgroup_id, interval_id))
                    existing_pairs.add((shift_id, workgroup_id, interval_id))
    ...
    for wg in workgroups:
        for worker_type in ['senior', 'junior']:
            ...
            sequences = WorkerSequence.query.join(Worker).filter(
                ...
                WorkerSequence.status =='initializing'
...
def export_schedule_by_shift():
    ...
    # Build data for each shift
    for shift in shifts:
        # Get workers assigned to this shift via schedules
        workers = [schedule.worker.name for schedule in shift.schedules]
    ...
    return send_file(
        ...
        mimetype='text/csv',
        as_attachment=True,
        download_name='schedule_by_shift.csv'
    )
...
def export_schedule_by_worker():
    ...
    # Build data for each worker
    for worker in workers:
        # Get shifts assigned to this worker via schedules
        shifts = [
            f"{schedule.shift.type}: {schedule.shift.day} {schedule.shift.start_time.strftime('%H:%M')}-{schedule.shift.end_time.strftime('%H:%M')}"
            for schedule in worker.schedules
        ]
        shifts_str = ', '.join(shifts) if shifts else 'None'
        data.append({
            'Worker ID': worker.id,
    ...
    return send_file(
        ...
        mimetype='text/csv',
        as_attachment=True,
        download_name='schedule_by_worker.csv'
    )
...
def can_choose_schedule(worker_id):
    ...
    # Check if all prior workers have a schedule
...
def schedule_system():
    ...
    if not can_choose_schedule(current_user.id):
        flash('It is not your turn to choose schedules yet.', 'error')
        return redirect(url_for('main.worker_dashboard'))
